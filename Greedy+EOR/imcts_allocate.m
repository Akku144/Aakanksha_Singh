...
while ~all(assigned)
    ...
    if isempty(actions)
        error('No actions generated. Check cfg.Ns, cfg.Nt, targets size, or proxy_cost output.');
    end
    ...
    for k = 1:Ns
        ...
        if j > 0 && ~assigned(j)
            ...
            dv = proxy_cost(ssc(k,:), targets0(j,:), dt, P, cfg.NmaxPhase);
...
for k = 1:cfg.Ns
    ...
    if j > 0
        ...
        dv = proxy_cost(ssc(k,:), targets(j,:), dt, P, cfg.NmaxPhase);
